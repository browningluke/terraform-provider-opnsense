...
// BindRecordResource defines the resource implementation.
...
func (r *BindRecordResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	resourceStruct, err := convertBindRecordSchemaToStruct(data)
	if err != nil {
...
func (r *BindRecordResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	// Get Record from OPNsense Bind API
	resourceStruct, err := r.client.Bind().GetRecord(ctx, data.Id.ValueString())
	if err != nil {
		var notFoundError *errs.NotFoundError
		if errors.As(err, &notFoundError) {
			tflog.Warn(ctx, fmt.Sprintf("Bind Record not present in remote, removing from state"))
	...
	resourceModel, err := convertBindRecordStructToSchema(resourceStruct)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read Bind Record, got error: %s", err))
		return
